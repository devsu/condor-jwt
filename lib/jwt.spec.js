...
describe('JWT', () => {
    ...
    describe('with getToken option', () => {
      ...
      it('should call getToken with the right arguments', (done) => {
        ...
        middleware(contextHelper.context, next).then(() => {
          ...
          expect(getToken).toHaveBeenCalledWith(contextHelper.context);
          done();
        });
      });
  ...
  it('should be extensible', (done) => {
    const Child = class extends JWT {};
    tokenHelper.setupValidToken();
    options = {'secretOrPublicKey': tokenHelper.keys.public};
    contextHelper.setupValidContext(tokenHelper.bearerTokenString);
    const child = new Child(options);
    child.getToken = Spy.returnValue({});
    child.isRevoked = Spy.returnValue(false);
    middleware = child.getMiddleware();
    middleware(contextHelper.context, next).then(() => {
      expect(child.getToken).toHaveBeenCalledTimes(1);
      expect(child.isRevoked).toHaveBeenCalledTimes(1);
      done();
    }).catch(done.fail);
  });
});
